        ...
        http
                ...
                .authorizeHttpRequests(requests -> {
                    requests
                            ...
                            .requestMatchers(GET, String.format("%s/users/profile-images/**", apiPrefix)).permitAll()

                            // Cart endpoints - require authentication or allow guest access
                            .requestMatchers(String.format("%s/cart/**", apiPrefix)).permitAll()

                            // Orders - require authentication (handled by @PreAuthorize)
                            .requestMatchers(String.format("%s/orders/**", apiPrefix)).authenticated()
                            .requestMatchers(String.format("%s/order_details/**", apiPrefix)).authenticated()

                            // Payment endpoints - require authentication
                            .requestMatchers(String.format("%s/payments/**", apiPrefix)).authenticated()

                            // Admin endpoints - will be handled by @PreAuthorize
                            .requestMatchers(String.format("%s/admin/**", apiPrefix)).authenticated()
